...
class PiEnsemble(Estimator):
    ...
    def __init__(self, alpha, ensemble_size, aggreg_func, hidden_size, learning_rate, epochs,
                 ...
                 retry_on_crossing=True, verbose=False, seed=None, output_dir=None, model = None,**kwargs):
        self._alpha = alpha
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray,
        ...
        while i <= self._ensemble_size:
            ...
            nn = PiNetwork(x_size=x_train.shape[1], y_size=3, h_size=self._hidden_size,
                           ...
                           summary_writers=summary, fmodel= self._fmodel,**self._kwargs)
...
class PiNetwork(Estimator):
    ...
    def __init__(self, x_size, y_size, h_size, alpha, epochs, batch_size, learning_rate, optimizer,
                 ...
                 print_frequency=100,fmodel =None, **kwargs):
        self._x_size = x_size
        self._h_size = h_size
        self._y_size = y_size
        self._alpha = alpha
        ...
        # self._model = LSTM(x_size, h_size, y_size)
        self._model = fmodel
        # self._model = GRUModel(x_size, h_size, y_size)
        # self._model = TransAm(feature_size=x_size, num_layers=1, dropout=0.1, nhead=7, max_len=124)
...
class RNNModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, dropout=0.1):
        ...
        # Output layers
        self.pi = nn.Linear(hidden_size * 2, output_size-1)  # The last dimension is for v
        self.v = nn.Linear(2, 1)

        # Initialization using RandomNormal and Constant
        nn.init.normal_(self.pi.weight, mean=0.0, std=0.2)
        self.pi.bias.data = torch.tensor([1., -1.])
    ...
    def forward(self, x):
        ...
        v = nn.functional.sigmoid(self.v(pi))
        vv = torch.cat((pi, v), dim=2)
        return vv
'''gru'''
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len):
        ...
        pe[:, 1::2] = torch.cos(position * div_term[0:(d_model - length)])
        pe = pe.unsqueeze(0).transpose(0, 1)
        self.register_buffer('pe', pe)

    def forward(self, x):
        x_len = x.size(1)
        pe = self.pe[:x_len, :, 0].T
        return x + pe

class MyTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dropout=0.1):
        super(MyTransformerEncoderLayer, self).__init__()
        self.self_attn = nn.Linear(d_model, d_model)  # Replace MultiheadAttention
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        # Calculate attention using linear layer
...
class TransAm(nn.Module):
    def __init__(self, feature_size=9, num_layers=1, dropout=0.1, nhead=7, max_len=124):
        super(TransAm, self).__init__()
        self.model_type = 'Transformer'
        self.pos_encoder = PositionalEncoding(feature_size, max_len)
        self.encoder_layer = MyTransformerEncoderLayer(d_model=feature_size, nhead=nhead, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=num_layers)
        # Output layers
        self.pi = nn.Linear(feature_size, 2)
        self.v = nn.Linear(feature_size, 1)

        # Initialization using RandomNormal and Constant
        nn.init.normal_(self.pi.weight, mean=0.0, std=0.2)
        self.pi.bias.data = torch.tensor([3., -3.])
        nn.init.xavier_normal_(self.v.weight)
        nn.init.constant_(self.v.bias, 0.0)
    ...
    def forward(self, src):
        ...
        src_padding_mask = (src.sum(dim=-1) == 0).transpose(0, 1).unsqueeze(1)

        # Apply transformer encoder with the added padding mask
        output = self.transformer_encoder(src, src_key_padding_mask=src_padding_mask)

        pi = self.pi(output)
        v = torch.sigmoid(self.v(output))
     
        return torch.cat((pi, v), dim=2)
